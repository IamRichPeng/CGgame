...
public class ShopPanel : MonoBehaviour
{
    ...
    private void OnBuyButtonClick()
    {
        ...
        {
            EventCenter.Broadcast(EventDefine.Hint, "No enough diamond");
    ...
    private void SetItemSize(int selectIndex)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            if (selectIndex == i)
            {
                ...
                parent.GetChild(i).GetChild(0).GetComponent<RectTransform>().localScale = new Vector3(1, 1, 1);
            }
            else
            {
                //parent.GetChild(i).GetChild(0).GetComponent<RectTransform>().sizeDelta = new Vector2(80, 80);
                parent.GetChild(i).GetChild(0).GetComponent<RectTransform>().localScale = new Vector3(0.5f, 0.5f, 1);
            }
        }
    }
